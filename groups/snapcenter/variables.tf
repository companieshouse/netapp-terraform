...
variable "snapcenter_data_volume_size" {
  type        = number
  description = "The size of the SnapCenter data volume in gibibytes (GiB) >= to the data volume defined in netapp-snapcenter-ami"
  default     = 200
}
...
variable "encrypt_ebs_block_device" {
  type        = bool
  description = "Defines whether the data EBS volume should be encrypted"
  default     = true
}

variable "ebs_block_device_iops" {
  type        = number
  description = "The required IOPS on the data EBS volume"
  default     = 3000
}

variable "ebs_block_device_throughput" {
  type        = number
  description = "The required data EBS volume throughput in MiB/s"
  default     = 125
}

variable "ebs_block_device_volume_type" {
  type        = string
  description = "The type of data EBS volume to provision"
  default     = "gp3"
}
